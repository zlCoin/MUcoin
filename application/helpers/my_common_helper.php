...
if ( ! function_exists('fun_electronic_type'))
{
	function fun_electronic_type()
	{
		...
		$_electronic_type['cash_to_electronic']="奖金积分转电子积分";
		...
		$_electronic_type['cash_currency_market']="奖金积分市场";
...
if ( ! function_exists('fun_cash_type'))
{
	...
	{
		$CI = & get_instance();
		$_cash_type['cash_to_electronic']="奖金积分转电子积分";
		$_cash_type['cash_to_shopping']="奖金积分转购物积分";
		$_cash_type['sell_cash']="奖金积分出售";
		$_cash_type['buy_cash']="奖金积分购买";
		$_cash_type['jihuo_sell'] = "激活账户扣除";
		$_cash_type['recommend']="推荐奖";
		$_cash_type['peace_prize'] = '平级奖';
		$_cash_type['guanli'] = '管理奖';
		$_cash_type['a_bonus'] = "积分赠送";
		$_cash_type['old_a_bonus'] = '老会员原始积分释放';
		$_cash_type['exchange'] = "积分兑换";
		$_cash_type['cash_to_wallet'] = "奖金转报单积分";
		$_cash_type['cash_to_cash'] = "报单积分互转";
		$_cash_type['cash_to_shopping_back']="奖金积分转购物积分失败";
		return $_cash_type;
	}
}
...
if ( ! function_exists('fun_cash_type'))
{
	function fun_baodan_type()
	{
		$CI = & get_instance();
		$_baodan_type['baodan_to_activation'] = "激活会员";
		$_baodan_type['cash_to_baodan'] = "奖金转报单";
		return $_baodan_type;
	}
}
...
if ( ! function_exists('fun_check_member_state'))
{
	function fun_check_member_state() { 
		$CI = & get_instance();
		$member_state = $CI->login_lib->member_state();
		if(!$member_state){
			$CI->session->set_flashdata('error_show', '账号未激活，不能操作！');
			redirect("/account");
		}
		return true;
	}
...
if ( ! function_exists('fun_switch_package'))
{
	...
	{
		$CI = & get_instance();
		$packages['0'] = array('version'=>'普通版','numbs'=>2000,'layer'=>2);
		$packages['1'] = array('version'=>'中级版','numbs'=>5000,'layer'=>4);
		$packages['2'] = array('version'=>'高级版','numbs'=>10000,'layer'=>6);
		$packages['3'] = array('version'=>'商务版','numbs'=>50000,'layer'=>8);
		$packages['4'] = array('version'=>'贵宾版','numbs'=>100000,'layer'=>10);
		return $packages;		
	}
}
...
if ( ! function_exists('fun_member_level'))
{
	...
	{
		$CI = & get_instance();
		$level['0'] = array('level'=>'1','numbs'=>3,'performance'=>100000,'proportion'=>0.1);	// 10W
		$level['1'] = array('level'=>'2','numbs'=>2,'performance'=>1000000,'proportion'=>0.15); // 100W
		$level['2'] = array('level'=>'3','numbs'=>3,'performance'=>5000000,'proportion'=>0.2); // 500W
		$level['3'] = array('level'=>'4','numbs'=>2,'performance'=>15000000,'proportion'=>0.25); // 1500W
		$level['4'] = array('level'=>'5','numbs'=>3,'performance'=>50000000,'proportion'=>0.3); // 5000W
		return $level;		
	}
}
