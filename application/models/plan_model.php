...
class  Plan_model extends Base_model {
	...
	function check_plan($bank_name,$account,$bank,$province,$city)
	{
		$con["where"][]=array('bank_name'=>$bank_name,'account'=>$account,'bank'=>$bank,'province'=>$province,'city'=>$city,'dtime'=>0);
        $this->set_args_array($con);
        return $this->fetch_count();
	}
	function plan_count_bymember_id($member_id)
	{
		$con["where"][]=array('member_id'=>$member_id,'dtime'=>0);
        $this->set_args_array($con);
        return $this->fetch_count();
	}
