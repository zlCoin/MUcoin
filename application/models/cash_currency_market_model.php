 ...
 * @filesource
 */
class Cash_currency_market_model extends Base_model {
    protected $_table_name = 'cash_currency_market';
    protected $_key_id = 'cash_currency_market_id';
	...
	function query_data($search=null,$limit="") 
	{
		...
		if($search['keyword']!="")
			$this->set_or_like('bank,user,buyer_user',$search['keyword']);
...
function query_data_state($search=null,$limit="") 
	{
		//$this->set_order_by('ctime,ASC');
		//caiqian 2016-01-31 随机排序
		$this->set_order_by('RAND(),ASC');
		if($search['keyword']!="")
			$this->set_or_like('member_id,user',$search['keyword']);
		if(isset($search['member_id']) && $search['member_id']!="")			
			$where['member_id']=$search['member_id'];
	...
	function check_state_count(){
		$con["where"][]=array('state !='=>2,'dtime'=>0);
        $this->set_args_array($con);
        return $this->fetch_count();	
	}
	function check_state_count_bymember_id($id=null){
		$con["where"][]=array('member_id '=>$id,'state !='=>2,'dtime'=>0);
        $this->set_args_array($con);
        return $this->fetch_count();	
	}
	function check_buyer_number($id=null){
		$t=time()-7*86400;
		$con["where"][]=array('member_id '=>$id,'cftime >='=>$t,'dtime'=>0);
        $this->set_args_array($con);
        return $this->fetch_count();	
	}
	function get_cash_currency_bymember_id($id=null){		
